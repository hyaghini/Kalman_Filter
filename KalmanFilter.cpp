...
KalmanFilter::KalmanFilter(
  ...
  : m_stateTransitionMatrix(stateTransitionMatrix),
    ...
    m_numStates(stateTransitionMatrix.rows()), isInitialized(false),
...
void KalmanFilter::predict(void) {
    ...
    if(m_stateTransitionMatrix.cols() != m_estimatedState.size())
    {
        throw std::runtime_error("Number of columns of the state transition matrix does not match number of states");
    }
